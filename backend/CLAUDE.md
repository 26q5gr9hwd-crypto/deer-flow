...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, VESPER context snapshot fields, and `vesper_delegation_runs`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear), `merge_delegation_runs` (merge per-task delegation records)
...
**Durable state**: `task_tool` writes a per-task record into `ThreadState.vesper_delegation_runs` with claim ownership, lineage, progress preview, and terminal status for Control Room / runtime introspection
